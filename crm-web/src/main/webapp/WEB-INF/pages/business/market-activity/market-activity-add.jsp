...
<div class="page-container">
    <form action="" method="post" class="form form-horizontal" id="form-article-add">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" name="" id="cost_ipt" placeholder="" value="" class="input-text" style="width:90%"> 元
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">开始时间：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" autocomplete="off" onfocus="WdatePicker({ minDate:'%y-%M-%d' })" id="datemin_ipt" class="input-text Wdate" style="width:180px;">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">结束时间：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" autocomplete="off" onfocus="WdatePicker({ minDate:'%y-%M-%d' })" id="datemax_ipt" class="input-text Wdate" style="width:180px;">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">活动描述：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <textarea id="description_ipt" name="" cols="" rows="" class="textarea" placeholder="说点什么...最少输入10个字符" datatype="*10-100" dragonfly="true" nullmsg="备注不能为空！" onKeyUp="$.Huitextarealength(this,200)"></textarea>
...
<script>
    ...
    function createActivity() {
        ...
        $.post('business/marketActivity/insert.do', params, function (result) {
            if (result.status == 'Success') {
                layer.msg('活动创建成功!', {icon: 1,time:2000});
            }else if(result.status == 'Fail') {
                layer.msg('活动创建失败，原因是：'+result.data.errorMsg, {icon: 2,time:2000,anim: 6},function () {
